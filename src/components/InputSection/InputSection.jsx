      ...
      if (resp.status === 200) {
        ...
        for (let i = 0; i < data.output.length; i++) {
          if(data.output[i].entity=="PERSON"){
            ...
            word=word.replace(data.output[i].name,`<span id="ORG"><a href="https://en.wikipedia.org/wiki/${data.output[i].name}" target="_blank">`+ data.output[i].name+`<span id="tag">DAT</span></a></span>`)
          }
